...
import eu.scape.bl.uk.normtabdata.characteriser.BLPDFFileCharacteriser;
import eu.scape.bl.uk.normtabdata.characteriser.FileCharacteriser;
import eu.scape.bl.uk.normtabdata.characteriser.GovDataFileCharacteriser;
import eu.scape.bl.uk.normtabdata.dto.DataFileDetailsDto;
import eu.scape.bl.uk.normtabdata.identifier.FileIdentifier;
import eu.scape.bl.uk.normtabdata.identifier.FileProcessor;
...
public class TabNormHadoopProcessor {
	...
	public TabNormHadoopProcessor () {
	}
	
	public TabNormHadoopProcessor (String propertiesFile, ArrayList<String> inputFiles) {
	...
	public String processRegister() throws Exception {
		...
		normalisedFormat = getNormalisationProperties();
		
		// Generate list of register files
		//ArrayList<String> registerFileList = generateListOfRegisterFiles();
			
		// Identify the type of each file
		Hashtable<String, DataFileDetailsDto> registerDTOs = identifyFileTypes(inputFiles);
		...
		String outputFile = listInputFiles(registerDTOs);
	...
	private Hashtable<String, DataFileDetailsDto> identifyFileTypes(ArrayList<String> registerFileList) {
		
		System.out.println("Identify file types");
		FileIdentifier identifier = new FileIdentifier(registerFileList, outputDirectory, auditDirectory);
	...
	private void characteriseRegisterFiles (Hashtable<String, DataFileDetailsDto> registerDTOs,
												NormalisedFormat normalisedFormat) 
												throws IOException, Exception {
					
		System.out.println("\n\nStarting to characterise register files");
		...
		while(dataFilesEnum.hasMoreElements()) {
			...
			if (Constants.FILE_MIME_TYPE_PLAIN_TEXT.equals(inputDataFile.getInputFileType()) ||
				...
				FileCharacteriser characteriser = 
						new GovDataFileCharacteriser(inputDataFile.getInputFilePath(), headerColumnKeywords);
		...
		System.out.println("Completed characterisation of register files");	
	}
	...
	private void processRegisterFiles (Hashtable<String, DataFileDetailsDto> registerDTOs,
		...
		while(dataFilesEnum.hasMoreElements()) {
			...
			FileProcessor registerProcessor = new FileProcessor(inputDataFileDetails, normalisedFormat);
	...
	private String listInputFiles(Hashtable<String, DataFileDetailsDto> registerDTOs) {	
		Enumeration<String> dataFilesEnum = registerDTOs.keys();
		String outputFile = "";
		while(dataFilesEnum.hasMoreElements()) {
			String key = dataFilesEnum.nextElement();
			...
			outputFile = dataFile.getOutputFilePath();
	...
	public ArrayList<String> getOutputDataItems() {
		return outputDataItems;
	}

	public void setOutputDataItems(ArrayList<String> outputDataItems) {
		this.outputDataItems = outputDataItems;
	}
	
	public NormalisedFormat getNormalisedFormat() {
		return normalisedFormat;
	}
}
